...
filtradoFinal = function(adjConectado,matrixx,stdd, perc){
...
###ya defini todo para el filtrado
			...
			infile = paste("Neighbourhood", perc,"k", kc, "std", stdd, ".png",sep="")
			png(filename=infile, 
    				units="in", 
    				width=10, 
    				height=8, 
    				pointsize=12, 
    				res=72)
			plot(genes,devSTD2[,1], pch="*", cex=2, col="black", xlab="Genes", ylab="",cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)# ylim = c(0,70), xlim = c(0,500)
			points(genes,devSTD2[,2],col="red", pch="o", cex=2)			
			legend(1,900,legend=c("Varianza vecinos","std"), col=c("black","red"),pch=c("*","o"), cex=1.5,box.lty = 2)
			text(100,1100,paste(" eliminados=",  percRechazado, "k=", kc), font=2, cex=1.5)
			dev.off()
			...
			matrixOut = matrix(0L, nrow=length(matrixx[,1]), ncol=length(matrixFinal[1,]) + 1)
			matrixOut[1:length(matrixFinal[,1]),1:length(matrixFinal[1,])]	= matrixFinal
			if (genesRechazados ==0){genesRechazados = rep(0,length(matrixFinal[,1]))} 
